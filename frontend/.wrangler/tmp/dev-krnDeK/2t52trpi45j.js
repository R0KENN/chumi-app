...
// .wrangler/tmp/pages-i5gkVe/functionsWorker-0.1255179365461837.mjs
...
var handleError = /* @__PURE__ */ __name2(async (error3, reject, options, namespace) => {
...
var LEVELS = [
  ...
  { level: 2, name: "Junior", nameRu: "\u041F\u043E\u0434\u0440\u043E\u0441\u0442\u043E\u043A", emoji: "\u{1F43E}", maxPoints: 63 },
  { level: 3, name: "Teen", nameRu: "\u042E\u043D\u044B\u0439", emoji: "\u{1F49C}", maxPoints: 90 },
  { level: 4, name: "Adult", nameRu: "\u0412\u0437\u0440\u043E\u0441\u043B\u044B\u0439", emoji: "\u{1F48E}", maxPoints: 135 },
  { level: 5, name: "Legend", nameRu: "\u041B\u0435\u0433\u0435\u043D\u0434\u0430", emoji: "\u{1F451}", maxPoints: 200 }
];
...
async function generateUniqueCode(supabase) {
  for (let attempt = 0; attempt < 20; attempt++) {
    ...
    const { data } = await supabase.from("pairs").select("code").eq("code", code).maybeSingle();
    if (!data) return code;
  }
...
async function sendTelegramMessage(env2, chatId, text, extra = {}) {
  try {
    ...
    if (!body.reply_markup) {
      body.reply_markup = {
        inline_keyboard: [[
          { text: "\u{1F43E} \u041E\u0442\u043A\u0440\u044B\u0442\u044C Chumi", web_app: { url: "https://chumi-app.pages.dev" } }
        ]]
      };
    }
...
function seededRandom(seed) {
  let s = seed >>> 0 || 1;
  return function() {
    s = Math.imul(s, 1103515245) + 12345 & 2147483647;
    return s / 2147483647;
  };
}
  ...
  try {
    ...
    if (request.method === "GET" && path.match(/^\/api\/pairs\/[^/]+$/)) {
      ...
      for (const up of userPairs) {
        ...
        const { data: tasks } = await supabase.from("daily_tasks").select("*").eq("pair_code", up.pair_code).eq("user_id", userId).eq("task_date", today);
        const { data: otTasks } = await supabase.from("one_time_tasks").select("task_key").eq("pair_code", up.pair_code).eq("user_id", userId);
    ...
    if (request.method === "GET" && path.match(/^\/api\/pair\/[^/]+\/[^/]+$/)) {
      ...
      const { data: tasks } = await supabase.from("daily_tasks").select("*").eq("pair_code", pairCode).eq("user_id", userId).eq("task_date", today);
      const { data: otTasks } = await supabase.from("one_time_tasks").select("task_key").eq("pair_code", pairCode).eq("user_id", userId);
    ...
    if (request.method === "GET" && path.match(/^\/api\/avatar\/[^/]+$/)) {
      ...
      try {
        const { data: cached } = await supabase.from("pair_users").select("avatar_file_path").eq("user_id", tgUserId).limit(1).maybeSingle();
        ...
        if (!filePath) {
          ...
          await supabase.from("pair_users").update({ avatar_file_path: filePath }).eq("user_id", tgUserId);
        }
        if (wantProxy === "1") {
          const avatarUrl = `https://api.telegram.org/file/bot${BOT_TOKEN}/${filePath}`;
          const imgRes = await fetch(avatarUrl);
          if (!imgRes.ok) {
            await supabase.from("pair_users").update({ avatar_file_path: null }).eq("user_id", tgUserId);
            return json({ avatar_url: null });
          }
    ...
    if (request.method === "GET" && path.match(/^\/api\/daily-tasks\/[^/]+\/[^/]+$/)) {
      ...
      const { data: pairTz } = await supabase.from("pairs").select("timezone").eq("code", pairCode).maybeSingle();
    ...
    if (request.method === "GET" && path.match(/^\/api\/streak-calendar\/[^/]+$/)) {
      ...
      const { data: pair } = await supabase.from("pairs").select("timezone, created_at").eq("code", pairCode).maybeSingle();
      if (!pair) return json({ error: "Pair not found" }, 404);
      const tz = pair.timezone || "UTC";
      const month = monthParam || getTodayDate(tz).slice(0, 7);
      const startDate = `${month}-01`;
      const [y, m] = month.split("-").map(Number);
      const lastDay = new Date(y, m, 0).getDate();
      const endDate = `${month}-${String(lastDay).padStart(2, "0")}`;
      const { data: members } = await supabase.from("pair_users").select("user_id").eq("pair_code", pairCode);
      if (!members || members.length === 0) return json({ days: [] });
      const memberIds = members.map((m2) => m2.user_id);
      const { data: opens } = await supabase.from("daily_tasks").select("user_id, task_date").eq("pair_code", pairCode).eq("task_key", "daily_open").gte("task_date", startDate).lte("task_date", endDate).in("user_id", memberIds);
      const byDate = {};
      for (const row of opens || []) {
        if (!byDate[row.task_date]) byDate[row.task_date] = /* @__PURE__ */ new Set();
    ...
    if (request.method === "GET" && path.match(/^\/api\/diary\/[^/]+$/)) {
      ...
      const { data: entries } = await supabase.from("pair_diary").select("id, user_id, emoji, text, entry_date, created_at").eq("pair_code", pairCode).order("entry_date", { ascending: false }).order("created_at", { ascending: true }).limit(200);
    ...
    if (request.method === "POST" && path === "/api/diary") {
      ...
      const { data: membership, error: memErr } = await supabase.from("pair_users").select("user_id, display_name").eq("pair_code", pairCode).eq("user_id", userId).maybeSingle();
      if (memErr) return json({ error: "Membership query failed: " + memErr.message }, 500);
      if (!membership) return json({ error: "Not a member" }, 403);
      const { data: pairTz } = await supabase.from("pairs").select("timezone, pet_name").eq("code", pairCode).maybeSingle();
      const today = getTodayDate(pairTz?.timezone || "UTC");
      const { data: existingToday } = await supabase.from("pair_diary").select("id").eq("pair_code", pairCode).eq("user_id", userId).eq("entry_date", today).maybeSingle();
      const isFirstEntryToday = !existingToday;
      const { error: upErr } = await supabase.from("pair_diary").upsert(
      ...
      if (isFirstEntryToday) {
        try {
          ...
          if (partner) {
            const { data: ps } = await supabase.from("user_settings").select("lang").eq("telegram_user_id", partner.user_id).maybeSingle();
            ...
            const notifyText = partnerLang === "ru" ? `\u{1F4D4} *${safeAuthor}* \u043E\u0441\u0442\u0430\u0432\u0438\u043B(\u0430) \u0437\u0430\u043F\u0438\u0441\u044C \u0432 \u0434\u043D\u0435\u0432\u043D\u0438\u043A\u0435 ${safePet}!

${emoji} _${safeText}_` : `\u{1F4D4} *${safeAuthor}* added a diary entry for ${safePet}!
...
${emoji} _${safeText}_`;
    ...
    if (request.method === "POST" && path === "/api/diary-delete") {
      ...
      const { data: entry } = await supabase.from("pair_diary").select("user_id").eq("id", entryId).maybeSingle();
    ...
    if (request.method === "POST" && path === "/api/create") {
      ...
      const userTz = typeof body.timezone === "string" && body.timezone.length < 64 ? body.timezone : "UTC";
      const maxPairs = await getMaxPairs(supabase, userId);
      const { data: existing } = await supabase.from("pair_users").select("pair_code").eq("user_id", userId);
      ...
      const { data: pending } = await supabase.from("pending_referrals").select("inviter_user_id").eq("invited_user_id", userId).maybeSingle();
    ...
    if (request.method === "POST" && path === "/api/complete-task") {
      ...
      if (points === void 0) return json({ error: "Invalid task" }, 400);
      const { data: pairCheck } = await supabase.from("pairs").select("is_dead, timezone, last_streak_date, streak_days, growth_points").eq("code", code).maybeSingle();
      ...
      if (pairCheck.is_dead && pairCheck.last_streak_date) {
        ...
        const lastDate = /* @__PURE__ */ new Date(pairCheck.last_streak_date + "T00:00:00Z");
        const todayDate = /* @__PURE__ */ new Date(todayCheck + "T00:00:00Z");
      ...
      if (taskKey === "add_to_home") {
        const { data: alreadyDone } = await supabase.from("one_time_tasks").select("id").eq("pair_code", code).eq("user_id", userId).eq("task_key", taskKey).maybeSingle();
      ...
      const { data: existing } = await supabase.from("daily_tasks").select("id").eq("pair_code", code).eq("user_id", userId).eq("task_key", taskKey).eq("task_date", today).maybeSingle();
      ...
      const { data: members } = await supabase.from("pair_users").select("user_id").eq("pair_code", code);
      const partnerIds = (members || []).map((m) => String(m.user_id)).filter((id) => id !== String(userId));
      if (taskKey === "daily_open") {
        const { data: pairForLife } = await supabase.from("pairs").select("last_streak_date").eq("code", code).single();
        if (pairForLife && pairForLife.last_streak_date !== today) {
          await supabase.from("pairs").update({ last_streak_date: today }).eq("code", code);
        }
      }
      let pointsAdded = 0;
      if (partnerIds.length > 0) {
        const memberIds = (members || []).map((m) => String(m.user_id));
        const { data: doneRows } = await supabase.from("daily_tasks").select("user_id").eq("pair_code", code).eq("task_key", taskKey).eq("task_date", today).in("user_id", memberIds);
        ...
        if (allMembersDone) {
          const { data: pair } = await supabase.from("pairs").select("growth_points, streak_days, last_streak_date, last_pair_streak_date, hatched").eq("code", code).single();
          if (pair) {
            ...
            if (taskKey === "daily_open" && pair.last_pair_streak_date !== today) {
              ...
              if (!prev) {
                ...
                const lastDate = /* @__PURE__ */ new Date(prev + "T00:00:00Z");
    ...
    if (request.method === "POST" && path === "/api/delete") {
      ...
      const { data: membership } = await supabase.from("pair_users").select("user_id").eq("pair_code", code).eq("user_id", userId).maybeSingle();
      if (!membership) return json({ error: "Not a member" }, 403);
      const { data: members } = await supabase.from("pair_users").select("user_id, display_name").eq("pair_code", code);
      for (const m of members || []) {
        if (m.user_id !== userId) {
          const { data: ps } = await supabase.from("user_settings").select("lang").eq("telegram_user_id", m.user_id).maybeSingle();
    ...
    if (request.method === "POST" && path === "/api/notify") {
      ...
      const { data: callerPairs } = await supabase.from("pair_users").select("pair_code").eq("user_id", userId);
      const { data: targetPairs } = await supabase.from("pair_users").select("pair_code").eq("user_id", targetUserId);
      ...
      const { data: recent } = await supabase.from("notification_log").select("id").eq("sender_user_id", userId).eq("target_user_id", targetUserId).gte("sent_at", oneHourAgo).limit(1);
    ...
    if (request.method === "POST" && path === "/api/recover-streak") {
      ...
      try {
        const { data: allMembers } = await supabase.from("pair_users").select("user_id, display_name").eq("pair_code", code);
        ...
        for (const p of partners) {
          const { data: ps } = await supabase.from("user_settings").select("lang").eq("telegram_user_id", p.user_id).maybeSingle();
          const pLang = ps?.lang || "ru";
          ...
          const safePet = escapeMd(petName);
          const text = pLang === "ru" ? `\u2728 *${safeReviver}* \u0432\u043E\u0441\u043A\u0440\u0435\u0441\u0438\u043B(\u0430) *${safePet}*! \u0421\u0435\u0440\u0438\u044F (${pair.streak_days} \u0434\u043D.) \u0441\u043E\u0445\u0440\u0430\u043D\u0435\u043D\u0430 \u{1F43E}
...
Revives left this month: ${5 - (used + 1)}/5.`;
    ...
    if (request.method === "GET" && path === "/api/ranking") {
      const { data: allPairs } = await supabase.from("pairs").select("code, pet_name, growth_points, streak_days").order("streak_days", { ascending: false }).order("growth_points", { ascending: false }).limit(100);
      const codes = (allPairs || []).map((p) => p.code);
      if (codes.length === 0) return json({ ranking: [] });
      const { data: allMembers } = await supabase.from("pair_users").select("pair_code, user_id, display_name, username").in("pair_code", codes);
      const userIds = [...new Set((allMembers || []).map((m) => m.user_id))];
      const nowIso = (/* @__PURE__ */ new Date()).toISOString();
      const { data: activeSubs } = await supabase.from("user_subscriptions").select("telegram_user_id").in("telegram_user_id", userIds).eq("status", "active").gt("expires_at", nowIso);
      const premiumSet = new Set((activeSubs || []).map((s) => String(s.telegram_user_id)));
      ADMIN_IDS.forEach((id) => premiumSet.add(String(id)));
      const membersByPair = /* @__PURE__ */ new Map();
    ...
    if (request.method === "GET" && path === "/api/ranking-random") {
      ...
      const dUtc = /* @__PURE__ */ new Date(todayUtc + "T00:00:00Z");
      dUtc.setUTCDate(dUtc.getUTCDate() - 2);
      const twoDaysAgoUtc = dUtc.toISOString().split("T")[0];
      const { data: allPairs } = await supabase.from("pairs").select("code, pet_name, growth_points, streak_days, last_streak_date, is_dead").not("pet_name", "is", null).eq("is_dead", false).gte("last_streak_date", twoDaysAgoUtc);
    ...
    if (request.method === "POST" && path === "/api/prepare-share") {
      ...
      const caption = userLang === "ru" ? `\u{1F43E} Chumi \u2014 \u0437\u0430\u0432\u0435\u0434\u0438 \u0432\u0438\u0440\u0442\u0443\u0430\u043B\u044C\u043D\u043E\u0433\u043E \u043F\u0438\u0442\u043E\u043C\u0446\u0430 \u0438 \u0440\u0430\u0441\u0442\u0438 \u0435\u0433\u043E \u0432\u043C\u0435\u0441\u0442\u0435 \u0441 \u0434\u0440\u0443\u0433\u043E\u043C!

\u0412\u044B\u043F\u043E\u043B\u043D\u044F\u0439 \u0437\u0430\u0434\u0430\u043D\u0438\u044F \u043A\u0430\u0436\u0434\u044B\u0439 \u0434\u0435\u043D\u044C, \u043F\u043E\u0434\u0434\u0435\u0440\u0436\u0438\u0432\u0430\u0439 \u0441\u0435\u0440\u0438\u044E \u0438 \u043E\u0442\u043A\u0440\u044B\u0432\u0430\u0439 \u043D\u043E\u0432\u044B\u0435 \u043E\u0431\u0440\u0430\u0437\u044B.` : `\u{1F43E} Chumi \u2014 get a virtual pet and grow it with a friend!
...
Complete tasks daily, keep your streak, unlock new outfits.`;
      ...
      const res = await fetch(`https://api.telegram.org/bot${env2.BOT_TOKEN}/savePreparedInlineMessage`, {
        ...
        body: JSON.stringify({
          ...
          result: {
            ...
            title: userLang === "ru" ? "Chumi \u2014 \u0412\u044B\u0440\u0430\u0441\u0442\u0438 \u043F\u0438\u0442\u043E\u043C\u0446\u0430! \u{1F43E}" : "Chumi \u2014 Grow a pet! \u{1F43E}",
            input_message_content: { message_text: caption },
            description: userLang === "ru" ? "\u0417\u0430\u0432\u0435\u0434\u0438 \u043F\u0438\u0442\u043E\u043C\u0446\u0430 \u0438 \u0440\u0430\u0441\u0442\u0438 \u0432\u043C\u0435\u0441\u0442\u0435 \u0441 \u0434\u0440\u0443\u0433\u043E\u043C \u{1F43E}" : "Get a pet and grow with a friend \u{1F43E}",
            reply_markup: { inline_keyboard: [[{ text: btnText, url: botLink }]] }
          },
          allow_user_chats: true,
          allow_bot_chats: false,
    ...
    if (request.method === "POST" && path === "/api/prepare-invite") {
      ...
      const { data: membership } = await supabase.from("pair_users").select("user_id").eq("pair_code", pairCode).eq("user_id", userId).maybeSingle();
      if (!membership) return json({ error: "Not a member" }, 403);
      const { data: ps } = await supabase.from("user_settings").select("lang").eq("telegram_user_id", userId).maybeSingle();
      const userLang = ps?.lang || "ru";
      const botUsername = env2.BOT_USERNAME || "ChumiPetBot";
...
\u0420\u0430\u0441\u0442\u0438\u043C \u043F\u0438\u0442\u043E\u043C\u0446\u0430 \u0432\u043C\u0435\u0441\u0442\u0435 \u2014 \u0432\u044B\u043F\u043E\u043B\u043D\u044F\u0439 \u0437\u0430\u0434\u0430\u043D\u0438\u044F, \u043F\u043E\u0434\u0434\u0435\u0440\u0436\u0438\u0432\u0430\u0439 \u0441\u0435\u0440\u0438\u044E \u0438 \u043E\u0442\u043A\u0440\u044B\u0432\u0430\u0439 \u043D\u043E\u0432\u044B\u0435 \u043D\u0430\u0440\u044F\u0434\u044B \u{1F423}` : `\u{1F43E} *Join my pair in Chumi!*
...
Let's grow a pet together \u2014 complete tasks, keep the streak, unlock outfits \u{1F423}`;
      ...
      const res = await fetch(`https://api.telegram.org/bot${env2.BOT_TOKEN}/savePreparedInlineMessage`, {
        ...
        body: JSON.stringify({
          ...
          result: {
            ...
            title: userLang === "ru" ? `\u041F\u0440\u0438\u0433\u043B\u0430\u0448\u0435\u043D\u0438\u0435 \u0432 Chumi: ${pairCode}` : `Chumi invite: ${pairCode}`,
            input_message_content: {
              message_text: messageText,
              parse_mode: "Markdown"
            },
            description: userLang === "ru" ? "\u0420\u0430\u0441\u0442\u0438\u0442\u0435 \u043F\u0438\u0442\u043E\u043C\u0446\u0430 \u0432\u043C\u0435\u0441\u0442\u0435 \u{1F43E}" : "Grow a pet together \u{1F43E}",
            reply_markup: {
    ...
    if (request.method === "POST" && path === "/api/prepare-task-message") {
      ...
      const btnText = userLang === "ru" ? "\u{1F43E} \u041E\u0442\u043A\u0440\u044B\u0442\u044C Chumi" : "\u{1F43E} Open Chumi";
      const titleByTask = {
        send_msg: userLang === "ru" ? "\u0421\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 Chumi \u{1F43E}" : "Chumi message \u{1F43E}",
        send_sticker: userLang === "ru" ? "\u0421\u0442\u0438\u043A\u0435\u0440 \u043E\u0442 Chumi \u{1F3A8}" : "Sticker from Chumi \u{1F3A8}",
    ...
    if (request.method === "POST" && path === "/api/send-reminders") {
      ...
      for (const pair of allPairs || []) {
        const today = getTodayDate(pair.timezone || "UTC");
        const { data: members } = await supabase.from("pair_users").select("user_id").eq("pair_code", pair.code);
        for (const member of members || []) {
          const { data: opened } = await supabase.from("daily_tasks").select("id").eq("pair_code", pair.code).eq("user_id", member.user_id).eq("task_key", "daily_open").eq("task_date", today).maybeSingle();
          if (!opened) {
            const todayStart = /* @__PURE__ */ new Date();
            todayStart.setUTCHours(0, 0, 0, 0);
            const { data: alreadySent } = await supabase.from("notification_log").select("id").eq("sender_user_id", "system_reminder").eq("target_user_id", member.user_id).gte("sent_at", todayStart.toISOString()).limit(1).maybeSingle();
            if (alreadySent) continue;
            const petName = pair.pet_name || "Chumi";
            const safePet = escapeMd(petName);
            const { data: ms } = await supabase.from("user_settings").select("lang").eq("telegram_user_id", member.user_id).maybeSingle();
            const mLang = ms?.lang || "ru";
            const reminderText = mLang === "ru" ? `\u{1F514} *${safePet}* \u0436\u0434\u0451\u0442 \u0442\u0435\u0431\u044F! \u0421\u0435\u0440\u0438\u044F: ${pair.streak_days} \u0434\u043D. \u{1F525}
...
Don't forget to come today!`;
    ...
    if (request.method === "POST" && path === "/api/update-streaks") {
      ...
      for (const pair of allPairs || []) {
        const tz = pair.timezone || "UTC";
        const today = getTodayDate(tz);
        const { data: pairMembers } = await supabase.from("pair_users").select("user_id").eq("pair_code", pair.code);
        ...
        if (pair.last_streak_date && pair.last_streak_date < yesterday) {
          ...
          killed++;
          const { data: deadMembers } = await supabase.from("pair_users").select("user_id").eq("pair_code", pair.code);
          for (const dm of deadMembers || []) {
            const { data: ps } = await supabase.from("user_settings").select("lang").eq("telegram_user_id", dm.user_id).maybeSingle();
            const dLang = ps?.lang || "ru";
            const petName = pair.pet_name || (dLang === "ru" ? "\u041F\u0438\u0442\u043E\u043C\u0435\u0446" : "Pet");
            const safePet = escapeMd(petName);
            const text = dLang === "ru" ? `\u{1F480} *${safePet}* \u0443\u043C\u0435\u0440... \u0421\u0435\u0440\u0438\u044F (${pair.streak_days} \u0434\u043D.) \u043F\u043E\u0434 \u0443\u0433\u0440\u043E\u0437\u043E\u0439!
\u0417\u0430\u0439\u0434\u0438 \u0432 \u043F\u0440\u0438\u043B\u043E\u0436\u0435\u043D\u0438\u0435 \u0438 \u043D\u0430\u0436\u043C\u0438 \xAB\u0412\u043E\u0441\u043A\u0440\u0435\u0441\u0438\u0442\u044C\xBB, \u0447\u0442\u043E\u0431\u044B \u043F\u0440\u043E\u0434\u043E\u043B\u0436\u0438\u0442\u044C \u0441\u0435\u0440\u0438\u044E.
\u041E\u0441\u0442\u0430\u043B\u043E\u0441\u044C \u0432\u043E\u0441\u043A\u0440\u0435\u0448\u0435\u043D\u0438\u0439 \u0432 \u044D\u0442\u043E\u043C \u043C\u0435\u0441\u044F\u0446\u0435: \u0434\u043E 5.` : `\u{1F480} *${safePet}* has died... Streak (${pair.streak_days} days) is at risk!
...
Up to 5 revivals per month available.`;
    ...
    if (request.method === "POST" && path === "/api/cleanup-empty-pairs") {
      ...
      for (const pair of allPairs || []) {
        ...
        if (!isEmpty) {
          const tz = pair.timezone || "UTC";
          const today = getTodayDate(tz);
          const todayD = /* @__PURE__ */ new Date(today + "T00:00:00Z");
          todayD.setUTCDate(todayD.getUTCDate() - 5);
          const fiveDaysAgoDate = todayD.toISOString().split("T")[0];
          const lastActivity = pair.last_streak_date || (pair.created_at ? pair.created_at.split("T")[0] : null);
    ...
    if (request.method === "POST" && path === "/api/admin-daily-summary") {
      ...
      const { count: totalPairs } = await supabase.from("pairs").select("code", { count: "exact", head: true });
      const { count: alivePairs } = await supabase.from("pairs").select("code", { count: "exact", head: true }).eq("is_dead", false);
      const { count: deadPairs } = await supabase.from("pairs").select("code", { count: "exact", head: true }).eq("is_dead", true);
      const { count: activeSubs } = await supabase.from("user_subscriptions").select("id", { count: "exact", head: true }).eq("status", "active").gt("expires_at", now.toISOString());
      const { count: totalSkinsOwned } = await supabase.from("user_skins").select("id", { count: "exact", head: true });
      const { count: newPairs24h } = await supabase.from("pairs").select("code", { count: "exact", head: true }).gte("created_at", yesterdayIso);
      const { count: newSubs24h } = await supabase.from("user_subscriptions").select("id", { count: "exact", head: true }).gte("starts_at", yesterdayIso);
      const { count: newSkins24h } = await supabase.from("user_skins").select("id", { count: "exact", head: true }).gte("created_at", yesterdayIso);
      const todayUtc = now.toISOString().split("T")[0];
      const { data: activeToday } = await supabase.from("daily_tasks").select("user_id").eq("task_key", "daily_open").eq("task_date", todayUtc);
      const activeUsersToday = new Set((activeToday || []).map((t) => t.user_id)).size;
      const { data: topStreaks } = await supabase.from("pairs").select("code, pet_name, streak_days").eq("is_dead", false).order("streak_days", { ascending: false }).limit(3);
      let topStreaksText = "\u2014";
      if (topStreaks && topStreaks.length > 0) {
        topStreaksText = topStreaks.map((p, i) => `${i + 1}. ${escapeMd(p.pet_name || "\u2014")} (${p.streak_days} \u0434\u043D.)`).join("\n");
      }
      const summary = `\u{1F4CA} *Chumi \u2014 \u0435\u0436\u0435\u0434\u043D\u0435\u0432\u043D\u0430\u044F \u0441\u0432\u043E\u0434\u043A\u0430*
_${now.toLocaleString("ru-RU", { dateStyle: "short", timeStyle: "short" })}_

\u{1F465} *\u0412\u0441\u0435\u0433\u043E \u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u0435\u0439:* ${totalUsers ?? 0}
\u{1F7E2} *\u0410\u043A\u0442\u0438\u0432\u043D\u044B\u0435 \u0441\u0435\u0433\u043E\u0434\u043D\u044F:* ${activeUsersToday}

\u{1F43E} *\u041F\u0430\u0440\u044B:* ${totalPairs ?? 0}
   \u2764\uFE0F \u0416\u0438\u0432\u044B\u0435: ${alivePairs ?? 0}
   \u{1F480} \u041C\u0451\u0440\u0442\u0432\u044B\u0435: ${deadPairs ?? 0}

*\u0417\u0430 \u043F\u043E\u0441\u043B\u0435\u0434\u043D\u0438\u0435 24 \u0447\u0430\u0441\u0430:*
   \u{1F195} \u041D\u043E\u0432\u044B\u0445 \u043F\u0430\u0440: ${newPairs24h ?? 0}
   \u2B50 \u041D\u043E\u0432\u044B\u0445 Premium: ${newSubs24h ?? 0}
   \u{1F3A8} \u041A\u0443\u043F\u043B\u0435\u043D\u043E \u0441\u043A\u0438\u043D\u043E\u0432: ${newSkins24h ?? 0}

\u{1F48E} *\u0410\u043A\u0442\u0438\u0432\u043D\u044B\u0445 Premium:* ${activeSubs ?? 0}
\u{1F3A8} *\u0421\u043A\u0438\u043D\u043E\u0432 \u0432 \u0441\u043E\u0431\u0441\u0442\u0432\u0435\u043D\u043D\u043E\u0441\u0442\u0438:* ${totalSkinsOwned ?? 0}
...
${topStreaksText}`;
    ...
    if (request.method === "POST" && path === "/api/send-partner-message") {
      ...
      const { data: membership } = await supabase.from("pair_users").select("user_id").eq("pair_code", code).eq("user_id", userId).maybeSingle();
      if (!membership) return json({ error: "Not a member" }, 403);
      const { data: rlMembers } = await supabase.from("pair_users").select("user_id").eq("pair_code", code);
      const rlPartner = (rlMembers || []).find((m) => String(m.user_id) !== String(userId));
      if (rlPartner) {
        const oneHourAgo = new Date(Date.now() - 60 * 60 * 1e3).toISOString();
        const { data: recent } = await supabase.from("notification_log").select("id").eq("sender_user_id", userId).eq("target_user_id", String(rlPartner.user_id)).gte("sent_at", oneHourAgo).limit(1);
        if (recent && recent.length > 0) {
          return json({ error: "Too many messages", retryAfter: 3600 }, 429);
        }
      }
      const { data: pairRow } = await supabase.from("pairs").select("pet_name, streak_days").eq("code", code).maybeSingle();
      ...
      const WEBAPP_URL2 = "https://chumi-app.pages.dev";
      const RU = [
        `\u{1F43E} \u041F\u0440\u0438\u0432\u0435\u0442! \u041D\u0435 \u0437\u0430\u0431\u044B\u0432\u0430\u0439 \u043F\u0440\u043E ${safePet} \u2014 \u0441\u0435\u0440\u0438\u044F ${streak} \u0434\u043D.!`,
        `\u{1F48C} \u0421\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u043E\u0442 \u043F\u0430\u0440\u0442\u043D\u0451\u0440\u0430 \u043F\u043E Chumi! \u041F\u0438\u0442\u043E\u043C\u0435\u0446 \u0440\u0430\u0441\u0442\u0451\u0442 \u0443\u0436\u0435 ${streak} \u0434\u043D. \u{1F43E}`,
        `\u{1F44B} ${safePet} \u0436\u0434\u0451\u0442 \u0442\u0435\u0431\u044F! \u0421\u0435\u0440\u0438\u044F: ${streak} \u0434\u043D. \u{1F43E}`
      ];
      const EN = [
        `\u{1F43E} Hey! Don't forget about ${safePet} \u2014 streak ${streak} days!`,
        `\u{1F48C} Message from your Chumi partner! Pet is growing for ${streak} days \u{1F43E}`,
        `\u{1F44B} ${safePet} is waiting! Streak: ${streak} days \u{1F43E}`
      ];
      for (const m of members || []) {
        ...
        await fetch(`https://api.telegram.org/bot${env2.BOT_TOKEN}/sendMessage`, {
          ...
          body: JSON.stringify({
            ...
            text,
            parse_mode: "Markdown",
            reply_markup: { inline_keyboard: [[{ text: btnText, web_app: { url: WEBAPP_URL2 } }]] }
          })
        });
        await supabase.from("notification_log").insert({
    ...
    if (request.method === "POST" && path === "/api/buy-skin-gift") {
      ...
      const { data: alreadyOwned } = await supabase.from("user_skins").select("id").eq("user_id", partner.user_id).eq("skin_id", skinId).maybeSingle();
    ...
    if (request.method === "POST" && path === "/api/set-skin") {
      ...
      if (skinId) {
        ...
        if (levelMatch) {
          ...
          const { data: pairData } = await supabase.from("pairs").select("growth_points").eq("code", pairCode).single();
          ...
          const premium = await isPremium(supabase, userId);
          if (!premium) {
            const { data: owned } = await supabase.from("user_skins").select("id").eq("user_id", userId).eq("skin_id", skinId).maybeSingle();
    ...
    if (request.method === "GET" && path.match(/^\/api\/premium\/[^/]+$/)) {
      ...
      if (premium) {
        const { data } = await supabase.from("user_subscriptions").select("expires_at").eq("telegram_user_id", userId).eq("status", "active").order("expires_at", { ascending: false }).limit(1).maybeSingle();
        expiresAt2 = data?.expires_at || null;
      }
      return json({ premium, expires_at: expiresAt2 });
    }
    if (request.method === "GET" && path.match(/^\/api\/recoveries-left\/[^/]+$/)) {
      ...
      const { data: pair } = await supabase.from("pairs").select("streak_recoveries_used, last_recovery_month, timezone").eq("code", pairCode).maybeSingle();
      if (!pair) return json({ error: "Pair not found" }, 404);
    ...
    if (request.method === "POST" && path === "/api/update-timezone") {
      ...
      const { data: myPairs } = await supabase.from("pair_users").select("pair_code").eq("user_id", userId);
      for (const up of myPairs || []) {
        const { data: members } = await supabase.from("pair_users").select("user_id").eq("pair_code", up.pair_code);
        const { data: pairRow } = await supabase.from("pairs").select("timezone").eq("code", up.pair_code).maybeSingle();
...
async function sendMessage(env2, chatId, text, extra = {}) {
  ...
  if (!body.reply_markup) {
    body.reply_markup = {
      inline_keyboard: [[
        { text: "\u{1F43E} \u041E\u0442\u043A\u0440\u044B\u0442\u044C Chumi", web_app: { url: WEBAPP_URL } }
      ]]
    };
...
var ADMIN_COMMANDS = [
  ...PUBLIC_COMMANDS,
  { command: "stats", description: "\u{1F4CA} \u0421\u0442\u0430\u0442\u0438\u0441\u0442\u0438\u043A\u0430 \u043F\u0440\u0438\u043B\u043E\u0436\u0435\u043D\u0438\u044F" },
  { command: "users", description: "\u{1F465} \u041F\u043E\u0441\u043B\u0435\u0434\u043D\u0438\u0435 \u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u0438" },
  { command: "summary", description: "\u{1F4C5} \u0415\u0436\u0435\u0434\u043D\u0435\u0432\u043D\u0430\u044F \u0441\u0432\u043E\u0434\u043A\u0430" },
  { command: "grantbee", description: "\u{1F41D} \u0412\u044B\u0434\u0430\u0442\u044C \u043D\u0430\u0440\u044F\u0434 \u041F\u0447\u0451\u043B\u043A\u0430 (USER_ID)" },
...
async function getMaxPairs2(supabase, userId) {
  ...
  if (sub && new Date(sub.expires_at) > /* @__PURE__ */ new Date()) return 999;
  const { data } = await supabase.from("user_slots").select("extra_slots").eq("telegram_user_id", userId).maybeSingle();
...
/lang \u2014 \u0441\u043C\u0435\u043D\u0438\u0442\u044C \u044F\u0437\u044B\u043A
/help \u2014 \u0441\u043F\u0440\u0430\u0432\u043A\u0430`, "welcome"),
    help: `\u{1F4D6} *\u041A\u043E\u043C\u0430\u043D\u0434\u044B:*
...
\u041E\u0442\u043F\u0440\u0430\u0432\u044C \u0434\u0440\u0443\u0433\u0443 \u0438\u043B\u0438 \u043D\u0430\u0436\u043C\u0438 \u043A\u043D\u043E\u043F\u043A\u0443 \u043D\u0438\u0436\u0435:`, "pairCreated"),
    maxPairs: /* @__PURE__ */ __name2((count3, max) => `\u26A0\uFE0F \u0423 \u0442\u0435\u0431\u044F ${count3}/${max} \u043F\u0430\u0440.`, "maxPairs"),
...
Send it to your friend or tap the button below:`, "pairCreated"),
    maxPairs: /* @__PURE__ */ __name2((count3, max) => `\u26A0\uFE0F You have ${count3}/${max} pairs.`, "maxPairs"),
  ...
  return {
    reply_markup: JSON.stringify({
      inline_keyboard: [
        ...
        [{
          text: lang === "ru" ? "\u{1F4E8} \u041F\u0440\u0438\u0433\u043B\u0430\u0441\u0438\u0442\u044C \u043F\u0430\u0440\u0442\u043D\u0451\u0440\u0430" : "\u{1F4E8} Invite partner",
          url: shareUrl
        }]
      ]
    })
...
var CUTE_MESSAGES = [
  ...
  "\u0422\u044B \u0434\u0435\u043B\u0430\u0435\u0448\u044C \u043C\u043E\u0439 \u0434\u0435\u043D\u044C \u043B\u0443\u0447\u0448\u0435 \u{1F308}",
  "\u041E\u0431\u043D\u0438\u043C\u0430\u044E \u0442\u0435\u0431\u044F \u043C\u044B\u0441\u043B\u0435\u043D\u043D\u043E \u{1F917}",
  "\u0422\u044B \u0441\u0430\u043C\u044B\u0439 \u043B\u0443\u0447\u0448\u0438\u0439 \u0447\u0435\u043B\u043E\u0432\u0435\u043A \u043D\u0430 \u0441\u0432\u0435\u0442\u0435 \u{1F496}",
  "\u0421\u043A\u0443\u0447\u0430\u044E \u043F\u043E \u0442\u0435\u0431\u0435 \u{1F97A}",
  "\u0422\u044B \u043C\u043E\u0439 \u043B\u044E\u0431\u0438\u043C\u044B\u0439 \u0447\u0435\u043B\u043E\u0432\u0435\u0447\u0435\u043A \u{1F497}",
  "\u0421\u043F\u0430\u0441\u0438\u0431\u043E \u0447\u0442\u043E \u0442\u044B \u0435\u0441\u0442\u044C \u{1F64F}\u{1F495}",
  "\u0425\u043E\u0447\u0443 \u043E\u0431\u043D\u044F\u0442\u044C \u0442\u0435\u0431\u044F \u043F\u0440\u044F\u043C\u043E \u0441\u0435\u0439\u0447\u0430\u0441 \u{1FAC2}",
  "\u0422\u044B \u0437\u0430\u0441\u043B\u0443\u0436\u0438\u0432\u0430\u0435\u0448\u044C \u0432\u0441\u0435\u0433\u043E \u0441\u0430\u043C\u043E\u0433\u043E \u043B\u0443\u0447\u0448\u0435\u0433\u043E \u2728",
  "\u0423\u043B\u044B\u0431\u043D\u0438\u0441\u044C, \u0442\u044B \u043F\u0440\u0435\u043A\u0440\u0430\u0441\u0435\u043D(\u043D\u0430) \u{1F60A}",
  "\u0422\u044B \u0441\u043E\u0433\u0440\u0435\u0432\u0430\u0435\u0448\u044C \u043C\u043E\u0451 \u0441\u0435\u0440\u0434\u0446\u0435 \u{1F493}",
  "\u041C\u043D\u0435 \u0442\u0430\u043A \u043F\u043E\u0432\u0435\u0437\u043B\u043E \u0447\u0442\u043E \u0442\u044B \u0443 \u043C\u0435\u043D\u044F \u0435\u0441\u0442\u044C \u{1F340}",
  "\u041F\u043E\u0441\u044B\u043B\u0430\u044E \u0442\u0435\u0431\u0435 \u043C\u043D\u043E\u0433\u043E \u043B\u044E\u0431\u0432\u0438 \u{1F48C}",
  "\u0422\u044B \u043C\u043E\u0439 \u0441\u0430\u043C\u044B\u0439 \u0431\u043B\u0438\u0437\u043A\u0438\u0439 \u0447\u0435\u043B\u043E\u0432\u0435\u043A \u{1FAF6}",
  "You are my sunshine \u2600\uFE0F",
  "Thinking of you \u{1F4AD}\u{1F495}",
  "You make my day better \u{1F308}",
  "Sending you a virtual hug \u{1F917}",
  "You're the best person ever \u{1F496}",
  "I miss you \u{1F97A}",
  "Thank you for being you \u{1F64F}\u{1F495}",
  "You deserve the best \u2728",
  "You warm my heart \u{1F493}",
  "I'm so lucky to have you \u{1F340}",
  "Sending lots of love \u{1F48C}",
  "You are my closest person \u{1FAF6}",
...
async function onRequestPost(context2) {
  ...
  try {
    ...
    if (update.inline_query) {
      ...
      const results = shuffled.map((text2, i) => ({
        type: "article",
        id: String(Date.now()) + "_" + i,
        title: text2,
        description: "\u{1F495}",
        input_message_content: { message_text: text2 }
      }));
    ...
    if (update.message?.successful_payment) {
      ...
      const userId2 = String(update.message.from.id);
      const lang2 = await getUserLang(supabase, userId2);
      ...
      if (payload.userId && String(payload.userId) !== userId2) {
        console.error("Payment payload userId mismatch:", payload.userId, "vs", userId2);
        return new Response("OK");
      }
      const pType = payload.type || payload.t || null;
      const pSkinId = payload.skinId || payload.s || null;
      const pRecipientId = payload.recipientId || payload.r || null;
      const pProductId = payload.productId || null;
      ...
      if (expected !== void 0 && payment.total_amount !== expected) {
        console.error(`Payment amount mismatch: got ${payment.total_amount}, expected ${expected}`);
        return new Response("OK");
      }
      if (chargeId) {
        const { data: dup } = await supabase.from("user_subscriptions").select("id").eq("telegram_payment_charge_id", chargeId).maybeSingle();
        if (dup) return new Response("OK");
      }
      if (pType === "skin" && pSkinId) {
        if (chargeId) {
          const { data: dup } = await supabase.from("processed_charges").select("charge_id").eq("charge_id", chargeId).maybeSingle();
          if (dup) return new Response("OK");
        }
        const { data: alreadyOwned } = await supabase.from("user_skins").select("id").eq("user_id", userId2).eq("skin_id", pSkinId).maybeSingle();
        if (!alreadyOwned) {
...
Charge: \`${chargeId || "\u2014"}\``
      ...
      if (pType === "skin_gift" && pSkinId && pRecipientId) {
        ...
        const skinName = pSkinId.charAt(0).toUpperCase() + pSkinId.slice(1);
        const { data: alreadyOwned } = await supabase.from("user_skins").select("id").eq("user_id", recipientId).eq("skin_id", pSkinId).maybeSingle();
        if (!alreadyOwned) {
          await supabase.from("user_skins").insert({
            user_id: recipientId,
            skin_id: pSkinId
...
They now own outfit *${escapeMd2(skinName)}* \u{1F3A8}`,
          webAppButton
        );
...
Open Chumi and try it on \u{1F43E}`,
          webAppButton
        );
        await notifyAdmins2(
          env2,
...
Charge: \`${chargeId || "\u2014"}\``
      ...
      if (pProductId === "extra_slot") {
        if (chargeId) {
          const { data: dupSlot } = await supabase.from("processed_charges").select("charge_id").eq("charge_id", chargeId).maybeSingle();
          if (dupSlot) return new Response("OK");
        }
        const { data: existing } = await supabase.from("user_slots").select("extra_slots").eq("telegram_user_id", userId2).maybeSingle();
        if (existing) {
          await supabase.from("user_slots").update({ extra_slots: (existing.extra_slots || 0) + 1 }).eq("telegram_user_id", userId2);
        } else {
...
Charge: \`${chargeId || "\u2014"}\``
      ...
      if (pProductId === "premium_monthly") {
        const now = /* @__PURE__ */ new Date();
        const { data: currentSub } = await supabase.from("user_subscriptions").select("expires_at").eq("telegram_user_id", userId2).eq("status", "active").order("expires_at", { ascending: false }).limit(1).maybeSingle();
        const baseTs = currentSub && new Date(currentSub.expires_at) > now ? new Date(currentSub.expires_at).getTime() : now.getTime();
        const expiresAt2 = new Date(baseTs + 30 * 24 * 60 * 60 * 1e3);
        await supabase.from("user_subscriptions").update({ status: "expired", updated_at: now.toISOString() }).eq("telegram_user_id", userId2).eq("status", "active");
        await supabase.from("user_subscriptions").insert({
          telegram_user_id: userId2,
...
\u0414\u0435\u0439\u0441\u0442\u0432\u0443\u0435\u0442 \u0434\u043E ${expiresAt2.toLocaleDateString("ru-RU")}` : `\u2B50 *Chumi Premium activated!*
...
Valid until ${expiresAt2.toLocaleDateString("en-US")}`,
          webAppButton
        );
...
ID: \`${userId2}\`
\u0421\u0443\u043C\u043C\u0430: \u2B50 ${payment.total_amount} Stars
\u0414\u0435\u0439\u0441\u0442\u0432\u0443\u0435\u0442 \u0434\u043E: ${expiresAt2.toLocaleDateString("ru-RU")}
Charge: \`${chargeId || "\u2014"}\``
        );
        return new Response("OK");
      }
    ...
    if (text === "/start" || text.startsWith("/start ")) {
      ...
      const { data: existingSettings } = await supabase.from("user_settings").select("telegram_user_id").eq("telegram_user_id", userId).maybeSingle();
      if (!existingSettings) {
        ...
        for (const adminId of ADMIN_IDS2) {
          ...
          try {
            await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
              ...
              body: JSON.stringify({
                chat_id: adminId,
                text: `\u{1F464} *\u041D\u043E\u0432\u044B\u0439 \u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044C!*

\u0418\u043C\u044F: ${escapeMd2(firstName)}
Username: ${username ? "@" + escapeMd2(username) : "\u2014"}
ID: \`${userId}\`
\u042F\u0437\u044B\u043A: ${lang}`,
                parse_mode: "Markdown"
              })
      ...
      if (startParam.startsWith("join_")) {
        ...
        const maxP = await getMaxPairs2(supabase, userId);
    ...
    if (text === "/stats") {
      ...
      const { count: alivePairs } = await supabase.from("pairs").select("code", { count: "exact", head: true }).eq("is_dead", false);
      const { count: deadPairs } = await supabase.from("pairs").select("code", { count: "exact", head: true }).eq("is_dead", true);
      const { count: activeSubs } = await supabase.from("user_subscriptions").select("id", { count: "exact", head: true }).eq("status", "active").gt("expires_at", (/* @__PURE__ */ new Date()).toISOString());
      const { count: totalSkins } = await supabase.from("user_skins").select("id", { count: "exact", head: true });
      const msg = `\u{1F4CA} *Chumi stats*

\u{1F465} Users: *${totalUsers ?? 0}*
...
\u{1F3A8} Skins owned: *${totalSkins ?? 0}*`;
    ...
    if (text === "/users") {
      ...
      const header = `\u{1F465} *\u0412\u0441\u0435\u0433\u043E \u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u0435\u0439: ${list.length}*
...
`;
    ...
    if (text.startsWith("/grantbee")) {
      ...
      const notifyText = targetLang === "ru" ? `\u{1F381} \u0422\u0435\u0431\u0435 \u043F\u043E\u0434\u0430\u0440\u0438\u043B\u0438 \u043D\u0430\u0440\u044F\u0434 *\u041F\u0447\u0451\u043B\u043A\u0430* \u{1F41D}!

\u041E\u0442\u043A\u0440\u043E\u0439 Chumi \u2192 *\u041D\u0430\u0440\u044F\u0434\u044B* \u2192 *\u041C\u0430\u0433\u0430\u0437\u0438\u043D* \u2014 \u043E\u043D \u0443\u0436\u0435 \u0443 \u0442\u0435\u0431\u044F.` : `\u{1F381} You've been gifted the *Bee* outfit \u{1F41D}!
...
Open Chumi \u2192 *Outfits* \u2192 *Shop* \u2014 it's yours.`;
    ...
    if (text.startsWith("/grantslot")) {
      ...
      const targetLang = await getUserLang(supabase, targetId);
      const notifyText = targetLang === "ru" ? `\u{1F381} \u0422\u0435\u0431\u0435 \u043F\u043E\u0434\u0430\u0440\u0438\u043B\u0438 \u0434\u043E\u043F\u043E\u043B\u043D\u0438\u0442\u0435\u043B\u044C\u043D\u044B\u0439 \u0441\u043B\u043E\u0442 \u0434\u043B\u044F \u043F\u0430\u0440\u044B!

\u0422\u0435\u043F\u0435\u0440\u044C \u0443 \u0442\u0435\u0431\u044F \u043D\u0430 1 \u043F\u0430\u0440\u0443 \u0431\u043E\u043B\u044C\u0448\u0435. \u041E\u0442\u043A\u0440\u043E\u0439 Chumi \u0438 \u0441\u043E\u0437\u0434\u0430\u0439 \u043D\u043E\u0432\u0443\u044E \u043F\u0430\u0440\u0443 \u{1F43E}` : `\u{1F381} You've been gifted an extra pair slot!
...
// .wrangler/tmp/pages-i5gkVe/2t52trpi45j.js
...
// .wrangler/tmp/bundle-ChjQ8f/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ChjQ8f/middleware-loader.entry.ts
