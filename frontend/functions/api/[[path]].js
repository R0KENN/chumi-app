...
function randomPetType() { return PET_TYPES[Math.floor(Math.random() * PET_TYPES.length)]; }
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (request.method === 'POST' && segments[0] === 'create') {
      ...
      if (!isAdmin && existing && existing.length >= MAX_PAIRS) return json({ success: false, message: `Max ${MAX_PAIRS} pairs allowed` });
      const code = generateCode();
      await supabase.from('pairs').insert({ code, pet_type: 'egg', hatched: false, streak_days: 0, growth_points: 0, last_streak_date: null, pet_name: null, bg_id: 'room' });
    ...
    if (request.method === 'POST' && segments[0] === 'rename') {
      ...
      const { data: link } = await supabase.from('pair_users').select('pair_code').eq('user_id', userId).eq('pair_code', pairCode).single();
    ...
    // POST /api/setbg — синхронизировать фон для пары
    if (request.method === 'POST' && segments[0] === 'setbg') {
      const { userId, pairCode, bgId } = await request.json();
      if (!userId || !pairCode || !bgId) return json({ success: false, message: 'userId, pairCode and bgId required' });
      const { data: link } = await supabase.from('pair_users').select('pair_code').eq('user_id', userId).eq('pair_code', pairCode).single();
      if (!link) return json({ success: false, message: 'Not in this pair' });
      await supabase.from('pairs').update({ bg_id: bgId }).eq('code', pairCode);
      return json({ success: true, bgId });
    }
    ...
    if (request.method === 'POST' && segments[0] === 'feed') {
      ...
      if (!link) return json({ success: false, message: 'Not in this pair' });
      const today = getTodayDate();
      const { data: existingFeed } = await supabase.from('feedings').select('id').eq('pair_code', pairCode).eq('user_id', userId).eq('fed_date', today).single();
      ...
      if (allFedToday) {
        ...
        if (!oldHatched && newStreak >= 3) { newHatched = true; justHatched = true; newPetType = randomPetType(); }
        const newStage = getStage(newPoints, newHatched);
        if (oldHatched && oldStage.name !== newStage.name) evolved = true;
        await supabase.from('pairs').update({ streak_days: newStreak, growth_points: newPoints, last_streak_date: today, hatched: newHatched, pet_type: newPetType }).eq('code', pairCode);
        pair.streak_days = newStreak; pair.growth_points = newPoints; pair.hatched = newHatched; pair.pet_type = newPetType;
      }
